...
export function useMedicalRecords() {
  ...
  const [historySummary, setHistorySummary] =
    useState<MedicalHistorySummary | null>(null);
  ...
  const getMedicalHistory = useCallback(async (): Promise<MedicalRecord[]> => {
    try {
      setLoading(true);
      setError(null);
      ...
      const mockRecords: MedicalRecord[] = [
        {
          ...
          diagnosis:
            "Overall good health, mild hypertension noted (140/90 mmHg)",
          treatment:
            "Lifestyle modifications: reduce sodium intake, increase exercise to 30min daily",
          notes:
            "Patient reports feeling well, no major concerns. Family history of cardiovascular disease.",
        ...
        },
        {
          ...
          prescription:
            "Lisinopril 10mg once daily in the morning, 30-day supply. Monitor BP weekly.",
          notes:
            "Patient counseled on medication adherence and potential side effects (dry cough, dizziness)",
          doctor: "Dr. Sarah Johnson, MD",
        ...
        },
        {
          ...
          diagnosis:
            "Stage 1 Hypertension, well-controlled with medication. BP: 125/80 mmHg",
          treatment:
            "Continue current medication regimen. Dietary consultation recommended.",
          notes:
            "Patient adherent to medication. Reports no side effects. Weight stable.",
          doctor: "Dr. Sarah Johnson, MD",
          facility: "City Medical Center",
  ...
  const getMedicalSummary =
    useCallback(async (): Promise<MedicalHistorySummary | null> => {
      try {
        setLoading(true);
        setError(null);
    ...
    try {
      ...
      if (summary) {
        ...
        context += `- Chronic Conditions: ${
          summary.chronic_conditions.join(", ") || "None reported"
        }\n`;
        context += `- Allergies: ${
          summary.allergies.join(", ") || "None reported"
        }\n`;
        context += `- Current Medications: ${
          summary.current_medications.join(", ") || "None"
        }\n`;
      ...
      if (records.length > 0) {
        ...
        records.slice(0, 5).forEach((record, index) => {
          context += `${index + 1}. ${
            record.date
          } - ${record.type.toUpperCase()}: ${record.description}\n`;
          if (record.diagnosis)
            context += `   Diagnosis: ${record.diagnosis}\n`;
          if (record.treatment)
            context += `   Treatment: ${record.treatment}\n`;
          if (record.prescription)
            context += `   Prescription: ${record.prescription}\n`;
      ...
      if (prescriptions.length > 0) {
        ...
        prescriptions
          ...
          .forEach((prescription, index) => {
            context += `${index + 1}. ${prescription.medication_name} ${
              prescription.dosage
            } - ${prescription.frequency}\n`;
            if (prescription.instructions)
              context += `   Instructions: ${prescription.instructions}\n`;
          });
        context += "\n";
      }
