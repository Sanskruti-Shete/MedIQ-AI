...
export function PatientRegistration({ onComplete }: PatientRegistrationProps) {
  ...
  const addItem = (
    field: "allergies" | "chronic_conditions" | "current_medications",
    value: string
  ) => {
    if (value.trim()) {
      setFormData((prev) => ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      await registerPatient(submissionData);
      onComplete?.();
    } catch (error) {
                    ...
                    {formData.chronic_conditions.map((condition, index) => (
                      ...
                      >
                        ...
                        <X
                          className="w-3 h-3 ml-1 cursor-pointer"
                          onClick={() =>
                            removeItem("chronic_conditions", index)
                          }
                        />
                      </Badge>
                    ))}
                    ...
                    {formData.current_medications.map((medication, index) => (
                      ...
                      >
                        ...
                        <X
                          className="w-3 h-3 ml-1 cursor-pointer"
                          onClick={() =>
                            removeItem("current_medications", index)
                          }
                        />
                      </Badge>
                    ))}
