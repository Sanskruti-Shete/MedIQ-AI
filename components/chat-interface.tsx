...
// Enhanced message rendering with medical styling
const MessageContent = ({ content, role }: { content: string; role: string }) => {
          ...
          return (
            <div key={index} className="mb-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-bold text-lg text-slate-800 font-serif">{header}</h3>
              </div>
              {content && (
                <div className="pl-4 border-l-2 border-emerald-200 text-slate-700 leading-relaxed">{content}</div>
              )}
            </div>
          )
        }
  ...
  if (role === "assistant") {
    return (
      <div className="prose prose-slate max-w-none">
        <div className="space-y-4 font-sans">{formatMedicalContent(content)}</div>
      </div>
    )
  }
  ...
  return (
    <div className="prose prose-slate max-w-none">
      <p className="text-slate-800 leading-relaxed whitespace-pre-wrap text-base font-medium">{content}</p>
    </div>
  )
}
...
export function ChatInterface({ chatId, onNewChat }: ChatInterfaceProps) {
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        setInput((prev) => prev + "[Voice message recorded - Speech-to-text integration ready]")
  ...
  const handleSubmit = async (message?: string) => {
    ...
    if ((!messageText && selectedFiles.length === 0) || isLoading) return
    ...
    try {
      ...
      const historyContext =
        medicalHistory.length > 0
          ? `\n\nPatient Medical History:\n${medicalHistory
              .map((record) => `- ${record.date}: ${record.diagnosis} (${record.treatment})`)
              .join("\n")}`
          : ""
      ...
      const response = await generateResponse(medicalContext + fullMessage)
      ...
      // Save medical interaction to records
      await addMedicalRecord({
        date: new Date().toISOString().split("T")[0],
        type: "consultation",
        description: messageText,
        diagnosis: "AI Consultation",
...
${result
  .map(
    (r: any, i: number) => `**${i + 1}. ${r.condition}** (${r.probability}% likelihood)
      ...
      <div className="flex flex-col h-full">
        <div className="border-b border-emerald-200/50 p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-emerald-900 font-serif">Symptom Analysis</h2>
            <Button variant="ghost" onClick={() => setShowSymptomAnalyzer(false)} className="hover:bg-emerald-100">
              <X className="w-4 h-4" />
            </Button>
          </div>
        </div>
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed font-light">
                    Your intelligent healthcare companion powered by advanced AI. Get instant medical insights, symptom
                    analysis, and personalized health guidance with access to your complete medical history.
                  </p>
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3 w-full">
                          <div className={cn("p-2 rounded-lg bg-white/80", prompt.color)}>
                            <IconComponent className="h-5 w-5" />
                          </div>
                          <div className="flex-1">
                            <p className="font-semibold text-slate-800">{prompt.text}</p>
                          ...
                          >
                            {copiedMessageId === `${index}` ? (
                              <>
                                ...
                                <span className="ml-1 text-xs text-green-600 font-medium">Copied!</span>
                              </>
                            ) : (
                              <>
          ...
          {showFilePreview && selectedFiles.length > 0 && (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-semibold text-emerald-700">Medical files attached:</span>
              </div>
