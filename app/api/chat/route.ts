...
Please provide a helpful, informative response that considers the patient's medical history (if provided) while maintaining appropriate medical disclaimers.`
    ...
    if (error?.message?.includes("API_KEY_INVALID")) {
      errorMessage = "The Gemini API key appears to be invalid. Please check your GEMINI_API_KEY environment variable."
    } else if (error?.message?.includes("QUOTA_EXCEEDED")) {
      errorMessage = "I've reached my usage limit for now. Please try again later or check your Gemini API quota."
